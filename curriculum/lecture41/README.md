...
## 2. 선수 조건
- lecture40 완료 또는 동등한 실무 경험
- Linux 기본 명령어(ls, cat, systemctl) 사용 가능

## 3. 학습 목표
- 로컬 실습과 AWS EC2/VPC/SG 매핑 주제를 Ansible 중심으로 설명할 수 있다.
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: 로컬 실습과 AWS EC2/VPC/SG 매핑 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
- 연계 목표: 로컬 실습과 AWS EC2/VPC/SG 매핑 학습 결과를 OpenStack 입문 체험 순서(MicroStack -> DevStack -> Kolla Ansible)와 연결한다.
### 추가 실습
- MicroStack에서 인스턴스/네트워크/보안그룹 흐름을 AWS EC2/VPC/SG와 1:1로 대응
- DevStack과 Kolla Ansible에서 CLI/컨테이너 관점으로 동일 리소스 생성 과정을 비교
- AWS/OpenStack 브릿지: OpenStack 학습 결과를 AWS EC2/VPC/SG/EBS 운영 시나리오로 직접 전이한다.
- 확장 프롬프트: 현재 강의(로컬 실습과 AWS EC2/VPC/SG 매핑) 결과를 MicroStack -> DevStack -> Kolla Ansible 순서로 확장하는 매핑 실습 체크리스트를 YAML로 작성해줘.
