...
## 2. 선수 조건
- lecture33 완료 또는 동등한 실무 경험
- Linux 기본 명령어(ls, cat, systemctl) 사용 가능

## 3. 학습 목표
- k3s 서버 노드 준비 자동화 설계 주제를 Ansible 중심으로 설명할 수 있다.
...
- 레퍼런스 플레이북: `../../playbooks/10_docker_engine_install.yml`
### 실행 명령
...
- `ansible-playbook -i ../../inventories/local/hosts.ini ../../playbooks/10_docker_engine_install.yml`
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: k3s 서버 노드 준비 자동화 설계 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
- 연계 목표: k3s 서버 노드 준비 자동화 설계 학습 결과를 k3s 대신 kubeadm 또는 managed EKS 확장 비교 확장 시나리오와 연결한다.
### 추가 실습
- 같은 앱의 k3s 배포 검증
- EKS에서 AWS 관리 영역과 사용자 관리 영역 분리 정리
- AWS/OpenStack 브릿지: k3s는 쿠버네티스 개념 학습용
- 확장 프롬프트: 현재 강의(k3s 서버 노드 준비 자동화 설계) 결과를 k3s 대신 kubeadm 또는 managed EKS 확장 비교 실습으로 확장하는 체크리스트를 YAML로 작성해줘.
