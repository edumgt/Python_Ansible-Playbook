...
## 3. 학습 목표
- Ansible 학습환경 진단과 컨트롤 노드 준비 주제를 Ansible 중심으로 설명할 수 있다.
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: Ansible 학습환경 진단과 컨트롤 노드 준비 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
- 연계 목표: Ansible 학습환경 진단과 컨트롤 노드 준비 학습 결과를 GitHub Actions 기반 CI 추가 확장 시나리오와 연결한다.
### 추가 실습
- backend push 시 테스트 실행
- 추후 ECR push 확장 가능한 workflow 설계
- AWS/OpenStack 브릿지: GitHub Actions -> ECR push -> ECS/EKS 배포 흐름으로 확장 가능
- 확장 프롬프트: 현재 강의(Ansible 학습환경 진단과 컨트롤 노드 준비) 결과를 GitHub Actions 기반 CI 추가 실습으로 확장하는 체크리스트를 YAML로 작성해줘.
