...
## 2. 선수 조건
- lecture02 완료 또는 동등한 실무 경험
- Linux 기본 명령어(ls, cat, systemctl) 사용 가능

## 3. 학습 목표
- Ad-hoc 명령과 연결 테스트 자동화 주제를 Ansible 중심으로 설명할 수 있다.
...
- 레퍼런스 플레이북: `../../playbooks/learning/01_ping.yml`
### 실행 명령
...
- `ansible-playbook -i ../../inventories/local/hosts.ini ../../playbooks/learning/01_ping.yml`
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: Ad-hoc 명령과 연결 테스트 자동화 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
- 연계 목표: Ad-hoc 명령과 연결 테스트 자동화 학습 결과를 모니터링(Prometheus/Grafana) 추가 확장 시나리오와 연결한다.
### 추가 실습
- k3s에 Prometheus/Grafana 배포
- 응답시간/에러율 시각화
- AWS/OpenStack 브릿지: CloudWatch, Managed Prometheus, Managed Grafana와 연결 가능
- 확장 프롬프트: 현재 강의(Ad-hoc 명령과 연결 테스트 자동화) 결과를 모니터링(Prometheus/Grafana) 추가 실습으로 확장하는 체크리스트를 YAML로 작성해줘.
