...
## 2. 선수 조건
- lecture26 완료 또는 동등한 실무 경험
- Linux 기본 명령어(ls, cat, systemctl) 사용 가능

## 3. 학습 목표
- VirtualBox 네트워크: NAT/Host-only/Bridged 주제를 Ansible 중심으로 설명할 수 있다.
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: VirtualBox 네트워크: NAT/Host-only/Bridged 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
- 연계 목표: VirtualBox 네트워크: NAT/Host-only/Bridged 학습 결과를 OpenStack 입문 체험 순서(MicroStack -> DevStack -> Kolla Ansible)와 연결한다.
### 추가 실습
- MicroStack에서 Horizon Networks/Routers/Floating IPs 화면을 확인하고 VirtualBox 네트워크 모드와 대응
- DevStack과 Kolla Ansible에서 VM NIC 연결 흐름과 Neutron 구성 요소를 비교
- AWS/OpenStack 브릿지: 로컬 네트워크 모드와 OpenStack Neutron 흐름을 AWS VPC/Subnet/Route/SG와 비교한다.
- 확장 프롬프트: 현재 강의(VirtualBox 네트워크: NAT/Host-only/Bridged) 결과를 MicroStack -> DevStack -> Kolla Ansible 순서로 확장하는 네트워크 매핑 체크리스트를 YAML로 작성해줘.
