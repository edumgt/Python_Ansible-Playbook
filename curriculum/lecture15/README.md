...
## 2. 선수 조건
- lecture14 완료 또는 동등한 실무 경험
- Linux 기본 명령어(ls, cat, systemctl) 사용 가능

## 3. 학습 목표
- Blue/Green 전환 자동화 주제를 Ansible 중심으로 설명할 수 있다.
...
- 레퍼런스 플레이북: `../../playbooks/13_blue_green_switch.yml`
### 실행 명령
...
- `ansible-playbook -i ../../inventories/local/hosts.ini ../../playbooks/13_blue_green_switch.yml`
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: Blue/Green 전환 자동화 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
- 연계 목표: Blue/Green 전환 자동화 학습 결과를 Nginx reverse proxy 심화 확장 시나리오와 연결한다.
### 추가 실습
- / -> frontend, /api -> backend 라우팅
- 보안 헤더 및 에러 페이지 추가
- AWS/OpenStack 브릿지: ALB + Nginx + Ingress Controller 구조 이해의 기반
- 확장 프롬프트: 현재 강의(Blue/Green 전환 자동화) 결과를 Nginx reverse proxy 심화 실습으로 확장하는 체크리스트를 YAML로 작성해줘.
