...
## 2. 선수 조건
- lecture01 완료 또는 동등한 실무 경험
- Linux 기본 명령어(ls, cat, systemctl) 사용 가능

## 3. 학습 목표
- Inventory 구조와 host/group 변수 설계 주제를 Ansible 중심으로 설명할 수 있다.
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: Inventory 구조와 host/group 변수 설계 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
...
- 연계 목표: Inventory 구조와 host/group 변수 설계 학습 결과를 Docker Compose 추가 확장 시나리오와 연결한다.
### 추가 실습
...
- 확장 프롬프트: 현재 강의(Inventory 구조와 host/group 변수 설계) 결과를 Docker Compose 추가 실습으로 확장하는 체크리스트를 YAML로 작성해줘.
