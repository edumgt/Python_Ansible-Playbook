...
## 2. 선수 조건
- lecture04 완료 또는 동등한 실무 경험
- Linux 기본 명령어(ls, cat, systemctl) 사용 가능

## 3. 학습 목표
- loop, when, block 제어문 실습 주제를 Ansible 중심으로 설명할 수 있다.
...
- 레퍼런스 플레이북: `../../playbooks/learning/03_loop_when.yml`
### 실행 명령
...
- `ansible-playbook -i ../../inventories/local/hosts.ini ../../playbooks/learning/03_loop_when.yml`
...
## 9. AI 페어링 가이드
- 사전 점검 프롬프트: loop, when, block 제어문 실습 실습 전에 실패할 수 있는 지점 3개를 체크리스트로 만들어줘.
- 실습 중 프롬프트: 현재 실행 로그를 보고 원인-증상-조치 순서로 트러블슈팅 가이드를 작성해줘.
- 회고 프롬프트: 이번 강의에서 배운 자동화 패턴을 다음 강의에 재사용할 수 있게 YAML 템플릿으로 정리해줘.

## 10. 심화 연계 (선택)
- 연계 트랙: `Nginx reverse proxy 심화`
- 연계 목표: loop, when, block 제어문 실습 학습 결과를 Nginx reverse proxy 심화 확장 시나리오와 연결한다.
### 추가 실습
- / -> frontend, /api -> backend 라우팅
- 보안 헤더 및 에러 페이지 추가
- AWS/OpenStack 브릿지: ALB + Nginx + Ingress Controller 구조 이해의 기반
- 확장 프롬프트: 현재 강의(loop, when, block 제어문 실습) 결과를 Nginx reverse proxy 심화 실습으로 확장하는 체크리스트를 YAML로 작성해줘.
